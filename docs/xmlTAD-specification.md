...
| P | ページ拘束（0:なし, 1:次段落1行目まで, 2:次付箋まで全段落） | "0" |
| height | 行高さ（SCALE: 比率は計算値、絶対は座標値） | "0.75", "150" |
| pargap | 段落間隔（SCALE: 同上） | "0.75" |
| left | 行頭マージン（座標系単位、signed） | "0", "50" |
| right | 行末マージン（座標系単位、signed） | "0", "30" |
| indent | 字下げ（座標系単位、signed） | "0", "20" |
| ntabs | タブストップ数（負の値は直前の付箋から継承） | "3", "-1", "0" |
| tabs | タブストップ位置のカンマ区切りリスト（ntabs>0の場合のみ） | "100,200,300" |
...
| 日付 | 版 | 内容 |
|------|-----|------|
| 2026-03-04 | 2.6 | `<mask>` data形式をBase64から16bit hexワード値（カンマ区切り）に変更、`<pattern>` pixelcolors廃止→TADパラメータ（ncol/fgcolors/bgcolor/masks）を必須に、`l_pat`を線カラーパターンIDとして明確化 |
| 2026-03-04 | 2.5 | `<mask>`/`<pattern>`のインライン配置形式追加（TADバイナリ出現順保持）、`<patterns>`ラッパーとの両形式読み込み対応 |
| 2026-03-04 | 2.4 | `<patterns>`形式改訂: `<mask>`要素追加、`<pattern>`要素をTADバイナリCOLORPATTERN構造に準拠（pixelcolors/ncol/fgcolors/bgcolor/masks属性） |
