...
TADjs Desktopは、BTRONのTAD（TRON Application Databus）形式ファイルを扱うためのデスクトップ環境です。文章と図形を統合的に扱える文書システムを、Electronベースのアプリケーションとして実現しています。

### 主な機能
- **TAD形式ファイルの表示・編集**: 文章と図形を含む文書の作成・編集
...
#### デスクトップ上での右クリックメニュー
- **小物**: 小物プラグインを起動
  - **ファイルを開く**: TAD/BPK/XTADファイルを開く
  - **システム環境設定**: システム全体の設定
  - **ユーザー環境設定**: ユーザー固有の設定
  - **屑実身操作**: 参照されていない実身の管理
  - **デスクトップ整理**: ウィンドウ位置の整理
- **ウィンドウ一覧**: 開いているウィンドウの一覧表示

#### 仮身上での右クリックメニュー
- **開く**: ファイルをデフォルトのプラグインで開く
- **実身についての操作**:
  - **実身の設定**: 実身の名前変更、アプリケーション設定
  - **実身の複製**: 実身をコピーして新しい実身を作成
- **プロパティ**: ファイルの詳細情報を表示
...
### 5. 基本表計算（Basic Calc Editor）

**プラグインID**: `basic-calc-editor`
**タイプ**: base
**説明**: スプレッドシート形式のデータを編集するプラグイン

#### 起動方法
- TAD/BPK/XTADファイルを右クリック → 「基本表計算」を選択

#### 機能
- **セル編集**: セルにテキストや数値を入力
- **書式設定**: セルの文字色、背景色、罫線などを設定
- **行列操作**: 行・列の挿入、削除、サイズ変更
- **仮身挿入**: セル内に仮身を配置

#### 操作方法
1. **セル選択**: セルをクリックして選択
2. **編集開始**: ダブルクリックまたはF2キーで編集モード
3. **確定**: Enterキーで確定

### 6. 予定表（Base Calendar）

**プラグインID**: `base-calendar`
**タイプ**: base
**説明**: カレンダー形式で予定や仮身を管理するプラグイン

#### 起動方法
- 原紙箱から「カレンダー」原紙をドラッグ＆ドロップ

#### 機能
- **月間表示**: カレンダー形式で日付を表示
- **自由仮身配置**: カレンダー上に仮身を自由配置
- **開いた仮身**: 仮身の内容をその場で展開表示
- **年月移動**: 前月/次月、前年/次年への移動

#### 操作方法
1. **日付セル範囲指定**: その日の実身を作成
2. **仮身のドラッグ**: カレンダー上に仮身を配置
3. **仮身の開閉**: 仮身をダブルクリックで展開

### 7. TADjs表示（TADjs View）
...
### 9. ファイルを開く（File Import）

**プラグインID**: `file-import`
**タイプ**: accessory
**説明**: 外部のTAD/BPK/XTADファイルを開きます

#### 起動方法
- デスクトップで右クリック → 小物 → ファイルを開く

#### 機能
- **ファイル選択ダイアログ**: システムのファイル選択ダイアログを表示
- **複数形式対応**: .tad, .bpk, .xtad ファイルを開く

### 10. 実身設定（Real Object Config）

**プラグインID**: `realobject-config`
**タイプ**: accessory
**説明**: 実身の名前やアプリケーション設定を変更します

#### 起動方法
- 仮身を右クリック → 実身についての操作 → 実身の設定

#### 機能
- **名前変更**: 実身の表示名を変更
- **アプリケーション設定**: デフォルトで開くプラグインを設定
- **メタデータ表示**: 実身ID、参照カウントなどを表示

### 11. 屑実身操作（Trash Real Objects）
...
#### 起動方法
- デスクトップで右クリック → 小物 → ユーザー環境設定

#### 機能
- **個人設定**: ユーザー固有の表示設定
- **デフォルトアプリ**: ファイル形式ごとのデフォルトプラグイン

### 14. 既存データ起動（Open Legacy App）

**プラグインID**: `open-legacy-app`
**タイプ**: base
**説明**: Excel、Word、PDFなど既存のWindowsアプリケーションでファイルを開きます

#### 起動方法
- 原紙箱から「既存データ起動」原紙をドラッグ＆ドロップ

#### 機能
- **外部アプリ起動**: Windowsにインストールされているアプリでファイルを開く
- **対応形式**: Excel (.xlsx), Word (.docx), PDF (.pdf) など

#### 使い方
1. 既存データ起動実身を基本文章編集で開く
2. 拡張子を記載（例: xlsx, docx, pdf）
3. 実身IDと同名のファイルを同じフォルダに配置
4. 仮身をダブルクリックで外部アプリ起動
...
#### 起動方法
- 原紙箱から「URL仮身」原紙をドラッグ＆ドロップ

#### 機能
- **URL記録**: 実身にURLを保存
- **ブラウザ起動**: ダブルクリックでブラウザを開く

#### 使い方
1. URL仮身を基本文章編集で開く
2. URLを記載（例: https://example.com）
3. 仮身をダブルクリックでブラウザ起動

### 16. デスクトップ整理（Desktop Organizer）

**プラグインID**: `desktop-organizer`
**タイプ**: accessory
**説明**: 開いているウィンドウの位置を整理します

#### 起動方法
- デスクトップで右クリック → 小物 → デスクトップ整理

#### 機能
- **カスケード配置**: ウィンドウを階段状に配置
- **タイル配置**: ウィンドウを並べて配置
- **全て最小化**: 全ウィンドウを最小化
...
#### 方法3: 小物メニュー
1. デスクトップで右クリック → 小物 → ファイル取込
2. ファイル選択ダイアログで TAD/BPK/XTAD ファイルを選択
...
### アプリが起動しない
1. Windows Defenderやウイルス対策ソフトの設定を確認
2. 管理者権限で実行を試す
3. フォルダ内の全ファイルが揃っているか確認
...
### Ver0.32 (2026-01)
- カレンダープラグインの開いた仮身表示対応
- 基本文章編集の段落間フォントスタイル継承対応
- xtadパース問題の修正
...
Copyright (c) 2025-2026 satromi

---

## お問い合わせ

- Email: satromi@gmail.com
