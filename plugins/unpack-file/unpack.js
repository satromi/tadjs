...
// ロガー初期化（モジュールスクリプトより先に実行されるためフォールバック）
const logger = (typeof window.getLogger === 'function') ? window.getLogger('UnpackFile') : console;
...
let currentFileIndex = 0;  // Track current file index for multiple tabs (tadFileIndex)
let currentLHeadIndex = 0;  // Track current lHead index for realIdMap/linkRecordList lookup
...
// 入れ子図形/文章開始セグメント用スタック（仕様準拠の状態管理）
// 各エントリ: { type, view, draw, hUnit, vUnit }
let figureStack = [];
...
let groupList = [];
let markerDefinitions = new Map(); // マーカー定義格納
...
class MaskData {
    constructor(id, hsize, vsize, maskBits) {
        ...
        this.bytesPerRow = Math.ceil(hsize / 8); // 1行あたりのバイト数（キャッシュ）
    }
    ...
    getMaskValue(x, y) {
        ...
        // バイト位置とビット位置を計算（bytesPerRowはコンストラクタでキャッシュ済み）
        const byteIndex = patternY * this.bytesPerRow + Math.floor(patternX / 8);
 ...
 * @param {number} colorByte - color値（0-255）
 * @returns {Object} { invert, palette, mode, raw }
 *   - invert: I bit (0=通常, 1=反転)
 *   - palette: P bit (0=直接方式, 1=カラーマップ方式)
 *   - mode: R bits (0=白黒, 1=RGB, 2=CMY)
 */
...
/**
 * cinfoエントリからピクセル値の指定ビット範囲を抽出
 * @param {number} pixelValue - ピクセル値
 * @param {number} cinfoEntry - cinfo (上位8=ビット位置, 下位8=ビット幅)
 * @returns {number} 抽出したチャンネル値
 */
...
/**
 * チャンネル値を8bitに拡張（bit replication）
 * @param {number} channelValue - チャンネル値
 * @param {number} bitWidth - 元のビット幅
 * @returns {number} 8bit値（0-255）
 */
function expandChannelTo8bit(channelValue, bitWidth) {
    ...
    // ビット拡張: 例 5bit (a4 a3 a2 a1 a0) → 8bit (a4 a3 a2 a1 a0 a4 a3 a2)
    let result = 0;
    let remaining = 8;
...
/**
 * 画像セグメントのバイト配列から指定オフセットのUH(2バイトLE)を読み取り
 */
...
/**
 * 画像セグメントのバイト配列から指定オフセットのUW(4バイトLE)を読み取り
 */
...
/**
 * 画像セグメントのバイト配列から指定オフセットのCOLOR(4バイト)を読み取り
 */
...
/**
 * マスクの最適 rowbytes を計算（仕様: 幅16以下=2, 32以下=4, 48以下=6）
 */
 ...
 */
function pass1() {
    logger.info('[Pass1] 実身UUID生成と管理用JSON生成開始');
    ...
    for (let i = 0; i < gHead.nfiles; i++) {
        ...
        logger.info(`[Pass1] 実身 ${i}: ${lhead.name} -> UUID: ${uuid}`);
    }

    logger.info(`[Pass1] 完了: ${realIdMap.size} 個の実身UUIDを生成`);
}
...
function pass2(lHead) {
    ...
    logger.info('[UnpackJS] forceXmlDumpEnabled を有効化しました');
    ...
    for (let i = 0; i < gHead.nfiles; i++) {
        ...
        for (let j = 0; j < lhead.f_nrec; j++) {
            ...
            if (rhead.type === 0) {
                ...
                // リンクレコードはXML不要
                logger.debug(`[UnpackJS] リンクレコードはXML生成不要`);
                ...
                try {
                    ...
                    if (isXmlDumpEnabled()) {
                        xml.push(''); // 仮のスロット予約
                        logger.debug(`[UnpackJS] TADレコード用にXMLスロット予約 xml[${xmlSlotIndex}]`);
                    }
                ...
                // 実行機能付箋レコードはXML不要
                logger.debug(`[UnpackJS] 実行機能付箋レコードはXML生成不要`);
                ...
                if (isXmlDumpEnabled() && enableAnotherRecord) {
                    ...
                    xml.push(binaryXml);
                    logger.debug(`[UnpackJS] その他のレコード(type=${rhead.type})用にバイナリXMLを追加 (xml.length = ${xml.length}, size=${rhead.size})`);
                }
            }
    ...
    for (let i = 0; i < tadRecordDataArray.length; i++) {
        ...
        if (tadFileIndex !== undefined) {
            ...
            currentLHeadIndex = lheadIndex;

            // TADレコード連番を設定（画像命名用: {realId}_{recordNo}_{imgNo}.png）
...
function processExtractedFiles(lHead) {
    logger.info('[UnpackJS] 解凍ファイル処理開始');

    // 初期化（realIdMapはpass1()で既に生成済みなのでクリアしない）
    // 注意: generatedImagesはpass2()のTADレコード処理中に蓄積されるためここではクリアしない
    archiveFiles = [];

    // window.xmlの内容をデバッグ出力
    if (typeof xml !== 'undefined') {
        logger.info('[UnpackJS] xml配列の要素数:', Array.isArray(xml) ? xml.length : 'not array');
    } else {
        logger.warn('[UnpackJS] xml配列が未定義です');
    }

    // lHeadから実身情報を取得
    if (typeof lHead !== 'undefined' && Array.isArray(lHead)) {
        logger.info('[UnpackJS] lHead の要素数:', lHead.length);
        ...
        lHead.forEach((lhead, index) => {
            ...
            if (!uuid) {
                logger.error(`[UnpackJS] 実身 ${index} のUUIDが見つかりません`);
                return;
            }

            logger.info(`[UnpackJS] 実身 ${index}: ${lhead.name} -> UUID: ${uuid} (Pass1で生成済み)`);

            // レコード数を取得
            const nrec = lhead.f_nrec || 1;
            logger.info(`[UnpackJS] 実身 ${index}: ${lhead.name}, レコード数: ${nrec}`);
            ...
            if (typeof xml !== 'undefined' && Array.isArray(xml)) {
                for (let recNo = 0; recNo < nrec; recNo++) {
                    ...
                    if (!hasXml) {
                        ...
                        logger.info(`[UnpackJS] 実身 ${index}, レコード ${recNo}: type=${recType} (XML不要)`);
                    } else {
                        // XMLありのレコードタイプ
                        if (xmlIndex < xml.length) {
                            recordXmls.push(xml[xmlIndex]);
                            logger.info(`[UnpackJS] 実身 ${index}, レコード ${recNo}: type=${recType}, xml[${xmlIndex}] (長さ: ${xml[xmlIndex] ? xml[xmlIndex].length : 0})`);
                            xmlIndex++;
                        } else {
                            logger.warn(`[UnpackJS] xml[${xmlIndex}] が範囲外です`);
                            recordXmls.push(null);
                        }
                    }
                }
    ...
    // 注意: generatedImagesはここではクリアしない (pass2で蓄積された画像データを
    //       後続のgenerateAndSendRealFiles()がgetGeneratedImages()経由で取得するため)
    //       クリアは送信完了後にclearGeneratedImages()を呼び出して行う
    tadRecordDataArray = [];
    tadRawDataArray = {};
    recordTypeMap = {};
    calcRecordTypeMap = {};
...
/**
 * generatedImagesをクリア（画像送信完了後に呼ぶ）
 */
function clearGeneratedImages() {
    generatedImages = [];
    logger.info('[UnpackJS] generatedImagesをクリアしました');
}
...
function tsTextEnd(tadSeg) {
    ...
    if (isXmlDumpEnabled() && isInDocSegment) {
        ...
        // 開いている段落タグがあれば閉じる
        if (isParagraphOpen) {
            logger.debug('tsTextEnd: Closing open paragraph tag');
            xmlBuffer.push('</p>\r\n');
            isParagraphOpen = false;
        }
...
function tsRulerTabFormatSetFusen(segLen, tadSeg) {
    ...
    // ATTRビット分解: RxxxxxPP
    const R = (ATTR >>> 7) & 0b1;       // 基準位置指定 (0:絶対, 1:相対)
    const P = ATTR & 0b11;              // ページ拘束 (0/1/2)

    // SCALE型データ
    const heightRaw = tadSeg[1];        // 行高さ
    const pargapRaw = tadSeg[2];        // 段落間隔

    // H型データ（符号付き変換）
    const left = uh2h(tadSeg[3]);       // 行頭マージン
    const right = uh2h(tadSeg[4]);      // 行末マージン
    const indent = uh2h(tadSeg[5]);     // 字下げ
    const ntabs = uh2h(tadSeg[6]);      // タブストップ数

    // タブストップ配列
    const tabs = [];
    if (ntabs > 0) {
        for (let i = 0; i < ntabs && (7 + i) < tadSeg.length; i++) {
            tabs.push(uh2h(tadSeg[7 + i]));
        }
    }
    ...
    if (isXmlDumpEnabled()) {
        ...
        let xml = `<tab-format R="${R}" P="${P}" height="${heightStr}" pargap="${pargapStr}"`;
        xml += ` left="${left}" right="${right}" indent="${indent}"`;
        xml += ` ntabs="${ntabs}"`;
        if (tabs.length > 0) {
            xml += ` tabs="${tabs.join(',')}"`;
        }
...
function tsImageSegment(segLen, tadSeg) {
    ...
    // 後方互換: 旧コード用に parseColor 結果も保持（既存の参照を残すため）
    imageSeg.color = parseColor(colorByte);

    // カラー情報配列 (cinfo[0..3]) - UH 4個
    ...
    logger.debug(`画像情報: ${width}x${height}, planes=${imageSeg.planes}, pixbits=0x${imageSeg.pixbits.toString(16)}, rowbytes=${imageSeg.rowbytes}`);
    ...
    logger.debug(`color=0x${colorByte.toString(16)}, I=${imageSeg.colorInfo.invert}, P=${imageSeg.colorInfo.palette}, R=${imageSeg.colorInfo.mode} (0=BW, 1=RGB, 2=CMY)`);
    logger.debug(`cinfo=[0x${imageSeg.cinfo[0].toString(16)}, 0x${imageSeg.cinfo[1].toString(16)}, 0x${imageSeg.cinfo[2].toString(16)}, 0x${imageSeg.cinfo[3].toString(16)}]`);
    logger.debug(`mask=0x${imageSeg.mask.toString(16)}, extlen=${imageSeg.extlen}, extend=0x${imageSeg.extend.toString(16)}`);

    // ★ セグメント全体のバイト配列を構築（オフセット計算用、0x00=Esc Sub Id 起点）
    // tadSeg[0] は仕様書オフセット 0x04 から始まる
    const segData = new Uint8Array(4 + tadSeg.length * 2);
    segData[0] = 0xFF;
    segData[1] = 0xE5;
    // segData[2..3]: Len（ダミー、参照されない）
    ...
    // ★ base_off[] (各プレーンのデータオフセット) を読み取り
    // 仕様オフセット 0x40 から planes 個の UW (4バイト)
    imageSeg.base_off = [];
    for (let p = 0; p < imageSeg.planes; p++) {
        imageSeg.base_off.push(readSegUW(segData, 0x40 + p * 4));
    }
    logger.debug(`base_off (raw)=[${imageSeg.base_off.map(o => '0x' + o.toString(16)).join(', ')}]`);

    // ★ base_off テーブル自身 (0x40 から planes*4 byte) を指している場合、
    // 実 plane データはテーブル直後から始まるため、テーブル終端まで進める。
    ...
    // ★ プレーンデータの抽出（互換性のため imageSeg.bitmap も維持）
    // imageSeg.bitmap = base_off[0] からの全データ（旧コード互換用）
    const planeOffsetStart = 30; // word index. 仕様オフセット = 0x04 + 30*2 = 0x40
    const imageDataStart = planeOffsetStart + imageSeg.planes * 2; // word index
...
/**
 * 単一プレーンから指定座標のピクセル値を取得（仕様準拠）
 * segData は セグメント先頭(0x00)からのバイト配列、planeOffset は base_off[] の値
 */
function readPixelFromPlane(imageSeg, planeOffset, srcX, srcY) {
    ...
    const pixelcount = imageSeg.pixbits & 0xFF;            // 有効ビット数
    const pixeldatawidth = (imageSeg.pixbits >> 8) & 0xFF; // 境界ビット数
    const stride = pixeldatawidth > 0 ? pixeldatawidth : pixelcount; // 実際のストライド bit
    const bytesPerPixel = Math.ceil(stride / 8);
    const rowbytes = imageSeg.rowbytes;

    if (stride <= 8) {
        // 1, 2, 4, 8 bit: MSB側が左のドット
...
/**
 * 多プレーンを結合してピクセル値を取得
 * 各プレーンのビットを連結（plane[0]=LSB, plane[N-1]=MSB）
 */
...
function getMaskValue(imageSeg, srcX, srcY) {
    if (!imageSeg.mask || imageSeg.mask === 0) return 1; // マスクなし=不透明
    const segData = imageSeg.segData;
    if (!segData) return 1;
...
/**
 * 仕様準拠版のピクセル色取得
 * 4方式（カラーマップ/直接白黒/直接RGB/直接CMY）に対応、I反転処理あり
 */
function getPixelColorSpecCompliant(imageSeg, srcX, srcY) {
    ...
    if (colorInfo.palette === 1) {
        ...
        // カラーマップ方式は反転無視（仕様: 反転は意味なし）
    } else if (colorInfo.mode === 0) {
        // ★ 直接白黒方式
        const cinfo0 = imageSeg.cinfo[0];
        // cinfo[0] が 0 の場合は pixbits の有効ビット幅で全体を1チャンネル扱い
        const cinfoEntry = cinfo0 || (imageSeg.pixbits & 0xFF);
        const bw = cinfoEntry & 0xFF || (imageSeg.pixbits & 0xFF) || 8;
        const luminance = expandChannelTo8bit(extractChannelValue(pixelValue, cinfoEntry || ((bw === 0 ? 8 : bw))), bw);
        // I=0: 値0=白, 最大値=黒 → 反転して RGB に
        // I=1: 値0=黒, 最大値=白 → そのまま輝度
...
function generatePngImage(imageSeg) {
    try {
        ...
        console.log('[PNG:DEBUG] Pixmap metadata:', {
            width, height,
            pixbits: '0x' + imageSeg.pixbits.toString(16),
            pixelcount, pixeldatawidth,
            planes: imageSeg.planes,
            rowbytes: imageSeg.rowbytes,
            bitmapLength: imageSeg.bitmap.length,
            colorMapLength: (typeof colorMap !== 'undefined' && colorMap) ? colorMap.length : 'N/A',
            firstBytes: Array.from(imageSeg.bitmap.slice(0, 16)).map(b => b.toString(16).padStart(2, '0')).join(' ')
        });

        // 実身ID（UUID）を取得
        // currentLHeadIndexからrealIdMapを使ってUUIDに変換
        const fileId = realIdMap.get(currentLHeadIndex) || `unknown_${currentLHeadIndex}`;
        ...
        logger.info(`[generatePngImage] Generating PNG: ${fileName} (${width}x${height})`);
        ...
        if (typeof document !== 'undefined') {
            ...
            // Canvas to PNG（同期的にDataURLからBlobに変換）
            ...
            logger.info(`[generatePngImage] PNG generated and stored: ${fileName} (${width}x${height})`);
            return fileName;
...
function tsFigEnd(tadSeg) {
    // セグメントスタックから図形セグメントを削除（仕様: 終了でスコープを pop）
    ...
    // 入れ子対応: figureStack からポップ
    if (figureStack.length > 0 && figureStack[figureStack.length - 1].type === 'figure') {
        figureStack.pop();
    }
...
function tsFigPolygonDraw(segLen, tadSeg) {
    ...
    const l_pat = Number(tadSeg[2]);
    const f_pat = Number(tadSeg[3]);
    // BTRON仕様: round=角丸め直径(UH), np=頂点数(UH, >2)
    const round = Number(uh2h(tadSeg[4]));
    const np = Number(uh2h(tadSeg[5]));

    logger.debug(`Polygon attributes: l_atr=${IntToHex((tadSeg[1]),4).replace('0x','')}, l_pat=${IntToHex((tadSeg[2]),4).replace('0x','')}, f_pat=${IntToHex((tadSeg[3]),4).replace('0x','')}, round=${round}, np=${np}`);

    if(isXmlDumpEnabled()) {
        const pointsArray = [];
        // np 個の頂点を読み取り（pt[i]: word index 6+i*2 が x、7+i*2 が y）
        for(let i = 0; i < np; i++) {
            const wordIdx = 6 + i * 2;
            if (wordIdx + 1 >= tadSeg.length) break;
            const px = Number(uh2h(tadSeg[wordIdx]));
            const py = Number(uh2h(tadSeg[wordIdx + 1]));
            pointsArray.push(`${px},${py}`);
        }
        figureZIndexCounter++;
        // round 属性も出力（角丸め直径、0=なし）
        xmlBuffer.push(`<polygon lineType="${lineType}" lineWidth="${lineWidth}" l_pat="${l_pat}" f_pat="${f_pat}" round="${round}" np="${np}" points="${pointsArray.join(' ')}" zIndex="${figureZIndexCounter}" />\r\n`);
        resetFigureModifier();
        resetFigureTransformState();
    }
}
...
function tsFigEllipseDraw(segLen, tadSeg) {
    ...
    // BTRON仕様: frame は half-open property、right/bottom はバイナリ上で実座標+1で格納される
    // XML には binary 由来の frame 値をそのまま出力 (cx/cy/rx/ry への変換はしない)
    ...
    if(isXmlDumpEnabled()) {
        figureZIndexCounter++;
        xmlBuffer.push(`<ellipse lineType="${lineType}" lineWidth="${lineWidth}" l_pat="${l_pat}" f_pat="${f_pat}" angle="${angle}" frameLeft="${frameLeft}" frameTop="${frameTop}" frameRight="${frameRight}" frameBottom="${frameBottom}" zIndex="${figureZIndexCounter}" />\r\n`);
        resetFigureModifier();
        resetFigureTransformState();
    }
}
...
function tsFigArcDraw(segLen, tadSeg) {
    ...
    // フレーム座標（バイナリ上は実座標+1の half-open。XML には binary 由来値をそのまま出力）
    const frameLeft = Number(uh2h(tadSeg[5]));
    const frameTop = Number(uh2h(tadSeg[6]));
    const frameRight = Number(uh2h(tadSeg[7]));
    const frameBottom = Number(uh2h(tadSeg[8]));
    ...
    if(isXmlDumpEnabled()) {
        ...
        xmlBuffer.push(`<arc lineType="${lineType}" lineWidth="${lineWidth}" l_pat="${l_pat}" f_pat="${f_pat}" angle="${angle}" frameLeft="${frameLeft}" frameTop="${frameTop}" frameRight="${frameRight}" frameBottom="${frameBottom}" startX="${startX}" startY="${startY}" endX="${endX}" endY="${endY}" start_arrow="${startArrow}" end_arrow="${endArrow}" arrow_type="simple" zIndex="${figureZIndexCounter}" />\r\n`);
        resetFigureModifier();
        resetFigureTransformState();
    }
}
...
function tsFigChordDraw(segLen, tadSeg) {
    ...
    if(isXmlDumpEnabled()) {
        ...
        figureZIndexCounter++;
        // XML 出力: frame + 生 PNT のみ。cx/cy/rx/ry/startAngle/endAngle は派生値のため出力しない
        xmlBuffer.push(`<chord lineType="${lineType}" lineWidth="${lineWidth}" l_pat="${l_pat}" f_pat="${f_pat}" angle="${angle}" frameLeft="${frameLeft}" frameTop="${frameTop}" frameRight="${frameRight}" frameBottom="${frameBottom}" startX="${startX}" startY="${startY}" endX="${endX}" endY="${endY}" start_arrow="${startArrow}" end_arrow="${endArrow}" arrow_type="simple" zIndex="${figureZIndexCounter}" />\r\n`);
        resetFigureModifier();
        resetFigureTransformState();
    }
}
...
function tsFigEllipticalArcDraw(segLen, tadSeg) {
    ...
    // BTRON仕様: frame は half-open property、right/bottom はバイナリ上で実座標+1で格納される
    // XML には binary 由来の frame 値をそのまま出力
    ...
    if(isXmlDumpEnabled()) {
        ...
        xmlBuffer.push(`<elliptical_arc lineType="${lineType}" lineWidth="${lineWidth}" l_pat="${l_pat}" angle="${angle}" frameLeft="${frameLeft}" frameTop="${frameTop}" frameRight="${frameRight}" frameBottom="${frameBottom}" startX="${startX}" startY="${startY}" endX="${endX}" endY="${endY}" start_arrow="${startArrow}" end_arrow="${endArrow}" arrow_type="simple" zIndex="${figureZIndexCounter}" />\r\n`);
        resetFigureModifier();
        resetFigureTransformState();
    }
}
...
function tsFigMarkerDraw(segLen, tadSeg) {
    ...
    if (isXmlDumpEnabled()) {
        const pointsArray = [];
        for (let i = 0; i < np; i++) {
            const px = Number(uh2h(tadSeg[3 + i * 2]));
            const py = Number(uh2h(tadSeg[4 + i * 2]));
            pointsArray.push(`${px},${py}`);
        }
        xmlBuffer.push(`<marker mode="${mode}" markerId="${markerId}" points="${pointsArray.join(' ')}" zIndex="${figureZIndexCounter}" />\r\n`);
    }
...
function tsFigColorPattern(segLen, tadSeg) {
    ...
    for (let j = 0; j < pattern.ncol; j++) {
        ...
        logger.debug(`fgCol[${j}]: ${fgCol.color}, transparent=${fgCol.transparent}`);
    }
    ...
    // XMLダンプ用にパターン定義を即時出力（TADバイナリの出現順を保持）
    // TAD仕様のパラメータのみ出力（pixelcolorsは廃止、読込側で再構築する）
    if (isXmlDumpEnabled()) {
        const fgcolors = pattern.fgcolArray.map(c => c.transparent ? 'transparent' : c.color).join(',');
        const bgcolor = pattern.bgcol.transparent ? 'transparent' : pattern.bgcol.color;
        const masks = pattern.mask.join(',');
        xmlBuffer.push(`<pattern id="${pattern.id}" type="0" width="${pattern.hsize}" height="${pattern.vsize}" ncol="${pattern.ncol}" fgcolors="${fgcolors}" bgcolor="${bgcolor}" masks="${masks}" />\r\n`);
    }
}
...
function tsMaskDataDefinition(segLen, tadSeg) {
    ...
    // XMLダンプ用にマスク定義を即時出力（TADバイナリの出現順を保持）
    // TAD仕様通り16bitワード値のカンマ区切りで出力
    if (isXmlDumpEnabled()) {
        const wordsPerRowMask = Math.ceil(hsize / 16);
        const wordValues = [];
        for (let row = 0; row < vsize; row++) {
            for (let w = 0; w < wordsPerRowMask; w++) {
                let word = 0;
                for (let bit = 0; bit < 16; bit++) {
                    const px = w * 16 + bit;
                    if (px >= hsize) break;
                    const srcByteIdx = row * bytesPerRow + Math.floor(px / 8);
                    const srcBitIdx = 7 - (px % 8);
                    if ((maskBits[srcByteIdx] >> srcBitIdx) & 1) {
                        word |= (1 << (15 - bit));
                    }
                }
        ...
        const hexValues = wordValues.map(v => v.toString(16).padStart(4, '0'));
        xmlBuffer.push(`<mask id="${id}" type="0" width="${hsize}" height="${vsize}" data="${hexValues.join(',')}" />\r\n`);
    }
...
function tsFigMarkerDefinition(segLen, tadSeg) {
    // 最小サイズチェック (type+id+size+fgCol×2 = 5 UHワード)
    if (segLen < 5) {
        return;
    }
...
function tsFigureTransformation(segLen, tadSeg) {
    ...
    if (segLen !== Number(0x0006) && segLen !== Number(0x0008) && segLen !== Number(0x000A)) {
        logger.debug("座標変換セグメント: セグメント長が不正 segLen=0x" + segLen.toString(16));
        return;
    }
    ...
    if (segLen >= Number(0x000A) && tadSeg[4] !== undefined) {
        ...
        if (vangle <= -90 || vangle >= 90) {
            logger.debug("座標変換セグメント: 傾斜角度が範囲外 vangle=" + vangle);
    ...
    logger.debug(`座標変換セグメント: dh=${dh}, dv=${dv}, hangle=${hangle}, vangle=${vangle}`);
}
...
function tsFigureModifier(segLen, tadSeg) {
    if (segLen < 1) {
        logger.debug("図形修飾セグメント: セグメント長が不正");
        return;
    }
    ...
    if (UB_SubID === Number(0x00)) {
        logger.debug("図形要素修飾セグメント");
        tsFigureArrowsModifier(segLen, tadSeg);
    } else if (UB_SubID === Number(0x01)) {
        logger.debug("座標変換セグメント");
...
function initTAD(x = 0, y = 0) {
    ...
    // TADブロック開始フラグをリセット（最初の文章/図形開始セグメントで解像度を取得するため）
    startTadSegment = false;
...
function tsVirtualObjSegment(segLen, tadSeg) {
    ...
    if (isProcessingBpk) {
        ...
        if (window.originalLinkId !== undefined && window.originalLinkId !== null) {
            ...
            // メインウィンドウの場合
            if (linkRecordList[currentLHeadIndex] && linkRecordList[currentLHeadIndex][linkNo]) {
                newLink = linkRecordList[currentLHeadIndex][linkNo];
            } else {
                newLink = new LINK();
            }
        }
    ...
    if (isXmlDumpEnabled()) {
        ...
        logger.info(`[tsVirtualObjSegment] link_id=${newLink.link_id}, linkedFileIndex=${linkedFileIndex}, realId=${realId}`);
    ...
    // linkRecordList[currentLHeadIndex]が存在しない場合は初期化
    if (!linkRecordList[currentLHeadIndex]) {
        logger.debug(`Initializing linkRecordList[${currentLHeadIndex}] as empty array`);
        linkRecordList[currentLHeadIndex] = [];
    }

    linkRecordList[currentLHeadIndex][linkNo] = newLink;
    linkNo++;
}
...
function tsSpecitySegmentCalc(dataSeg) {
    ...
    if (isXmlDumpEnabled()) {
        ...
        logger.debug(`calcPos cell=${cellRef} (col=${col} row=${row})`);
        ...
        // デバッグ: 生データをダンプ
        logger.info(`[tsSpecitySegmentCalc] cell=${cellRef} dataSeg[4]=0x${dataSeg[4].toString(16).padStart(4,'0')} dataSeg[5]=0x${dataSeg[5].toString(16).padStart(4,'0')}`);
        logger.info(`  fontSizeCode=0x${fontSizeCode.toString(16)} deco=0x${deco.toString(16)} border=0x${border.toString(16)} colorCode=0x${colorCode.toString(16)}`);
...
function tsSpecitySegment(segLen, tadSeg, nowPos) {
    ...
    if (calcAppId1 == tadSeg[12] && calcAppId2 == tadSeg[13] && calcAppId3 == tadSeg[14]) {
        logger.debug("基本表計算形式の アプリケーションID");
...
function tadPerse(segID, segLen, tadSeg, nowPos) {
    ...
    if (segID === Number(TS_INFO)) {
        ...
        logger.info('Virtual object segment detected! segLen:', segLen, 'currentFileIndex:', currentFileIndex);
...
function tadDataArray(raw, isRedrawn = false, nfiles = null, fileIndex = null) {
    ...
    if (isXmlDumpEnabled()) {
        ...
        if (currentXmlSlotIndex >= 0) {
            ...
            logger.debug(`XML stored in reserved slot xml[${currentXmlSlotIndex}] for file ${fileIndex}: ${parseXML.substring(0, 100)}...`);
        } else {
            // 通常の場合はpushで追加
            xml.push(parseXML);
...
async function parseTADToXML(rawData, fileIndex = 0) {
    return new Promise((resolve, reject) => {
        try {
            logger.info('parseTADToXML開始: fileIndex=' + fileIndex + ', データサイズ=' + rawData.length);
            ...
            logger.info('tadRawArray呼び出し前: isXmlDumpEnabled=' + isXmlDumpEnabled());

            // TADをパース（同期処理）
            tadRawArray(uint8Array);

            logger.info('tadRawArray呼び出し後: xml.length=' + xml.length + ', xml[' + fileIndex + '].length=' + (xml[fileIndex] ? xml[fileIndex].length : 0));
            logger.info('parseXML.length=' + parseXML.length);

            // パース結果のXMLを取得
            let resultXML = '';
            if (xml && xml[fileIndex]) {
                resultXML = xml[fileIndex];
                logger.info('xml[' + fileIndex + ']から取得: ' + resultXML.length + '文字');
            } else if (parseXML) {
                resultXML = parseXML;
                logger.info('parseXMLから取得: ' + resultXML.length + '文字');
            }

            // 強制フラグを戻す
            forceXmlDumpEnabled = false;

            logger.info(`TAD→XML変換完了: ${resultXML.length}文字`);
            if (resultXML.length > 0) {
                logger.info('XMLプレビュー: ' + resultXML.substring(0, 200));
            }
            resolve(resultXML);
        } catch (error) {
