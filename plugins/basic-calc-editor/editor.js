...
class CalcEditor extends window.CalcChartMixin(window.PluginBase) {
    constructor() {
        // 基底クラスのコンストラクタを呼び出し
        super('CalcEditor');
     ...
     * @returns {string} "col,row" 形式のキー文字列
     */
    getCellKey(col, row) {
        return `${col},${row}`;
    }
    ...
    init() {
        ...
        // 共通コンポーネントの初期化（VirtualObjectRenderer, IconCacheManager, スクロールバーCSS変数）
        this.initializeCommonComponents('[CalcEditor]');
    ...
    setupMessageBusHandlers() {
        ...
        this.messageBus.on('init', async (data) => {
            ...
            if (data.fileData) {
                // realIdから拡張子・インデックスを除去（PluginBase共通メソッド使用）
    ...
    selectCell(col, row, clearRange = true) {
        ...
        const cellData = this.cells.get(key);
        if (cellData && cellData.contentType === 'virtualObject' && cellData.virtualObject) {
            // 実身IDを抽出（PluginBase共通メソッド使用）
            const realId = this.extractRealId(cellData.virtualObject.link_id);
                ...
                if (this.virtualObjectRenderer) {
                    ...
                    vobjElement.addEventListener('contextmenu', (e) => {
                        ...
                        this.contextMenuVirtualObject = {
                            ...
                            realId: realId,
                            virtualObj: cellData.virtualObject,
                            cellKey: this.getCellKey(col, row)
                        };
    ...
    evaluateFormula(formula, col, row, evaluating = new Set()) {
        try {
            ...
            const allowedFunctions = {
                ...
                IF: formulajs.IF || function(condition, trueVal, falseVal) { return condition ? trueVal : falseVal; },
                ROUND: formulajs.ROUND || Math.round,
                FLOOR: formulajs.FLOOR || Math.floor,
                CEIL: formulajs.CEILING || Math.ceil,
            ...
            // 危険な識別子をブロック（コードインジェクション防止）
            const dangerousPattern = /\b(require|import|process|window|document|global|globalThis|self|top|parent|frames|constructor|__proto__|prototype|eval|Function|setTimeout|setInterval|setImmediate|fetch|XMLHttpRequest|WebSocket|Worker|Blob|module|exports|child_process|fs|path|os|net|http|https|crypto)\b/;
            if (dangerousPattern.test(expr)) {
                logger.warn(`[CalcEditor] 危険な式を検出: ${expr}`);
                return '#ERROR!';
            }

            // 安全な数式評価: new Function + strict mode + 制限されたスコープ
            const funcParams = Object.keys(allowedFunctions);
            const funcArgs = Object.values(allowedFunctions);
            const safeFunc = new Function(...funcParams, '"use strict"; return (' + expr + ');');
            const result = safeFunc(...funcArgs);
            return result;
        } catch (e) {
    ...
    async pasteImageCell(key, cellInfo, targetCol, targetRow) {
        ...
        // 新しいファイル名を生成
        const realId = this.extractRealId(this.realId || 'unknown');
        const recordNo = 0;
    ...
    async preloadVirtualObjectIcons(xtadData) {
        ...
        while ((match = linkRegex.exec(xtadData)) !== null) {
            ...
            if (attrMap.id) {
                // フルIDから実身IDのみを抽出（PluginBase共通メソッド使用）
                const realId = this.extractRealId(attrMap.id);
                realIds.push(realId);
            }
        }
    ...
    async handleDoubleClickDragDuplicate(sourceCol, sourceRow, targetCol, targetRow) {
        ...
        const realId = this.extractRealId(virtualObject.link_id);
