...
const SELECTION_BOX_SHADOW = `0 0 0 2px ${VOBJ_SELECTION_COLOR}`;  // 選択時のbox-shadow
...
class VirtualObjectListApp extends window.PluginBase {
    constructor() {
        ...
        this.vobjDragState = {
            ...
            startScrollLeft: 0,          // ドラッグ開始時のスクロール位置X
            startScrollTop: 0            // ドラッグ開始時のスクロール位置Y
        };
    ...
    insertVirtualObjectFromDrag(dragData, clientX, clientY) {
        ...
        virtualObjects.forEach((virtualObjectData, index) => {
            ...
            if (!width || !heightPx) {
                ...
                heightPx = heightPx || (vobjbottom - vobjtop) || DEFAULT_VOBJ_HEIGHT; // デフォルト（閉じた仮身の標準高さ）
            }
    ...
    async addVirtualObjectFromFile(file) {
        try {
            ...
            const newVirtualObject = {
                ...
                frcol: DEFAULT_FRCOL,   // デフォルト枠色
                chcol: DEFAULT_CHCOL,   // デフォルト文字色
                tbcol: DEFAULT_TBCOL,   // デフォルト背景色
                bgcol: DEFAULT_BGCOL,   // デフォルト開いた仮身の背景色
    ...
    async handleDoubleClickDragDuplicate(dropX, dropY) {
        ...
        const chsz = obj.chsz || DEFAULT_FONT_SIZE;
        const frcol = obj.frcol || DEFAULT_FRCOL;
        const chcol = obj.chcol || DEFAULT_CHCOL;
        const tbcol = obj.tbcol || DEFAULT_TBCOL;
        const bgcol = obj.bgcol || DEFAULT_BGCOL;
    ...
    async showArrangeDialog() {
        ...
        if (result && result.button === 'ok') {
            ...
            logger.debug('[VirtualObjectList] OKボタンが押されませんでした。button:', result?.button);
        }
    ...
    /**
     * ドラッグ中のエッジスクロールを実行
     * マウスがウィンドウ端（10px以内）にある場合、その方向にスクロール
     * @param {DragEvent} e - ドラッグイベント
     */
    performEdgeScroll(e) {
        const pluginContent = document.querySelector('.plugin-content');
        const canvas = document.querySelector('.virtual-canvas');
        if (!pluginContent || !canvas) return;

        // ウィンドウ（iframe）の実際のサイズを使用
        // pluginContentのrectではなくウィンドウ境界でエッジ判定を行う
        const windowWidth = window.innerWidth;
        const windowHeight = window.innerHeight;
        const threshold = 30;  // エッジ検出閾値（px）- ウィンドウ枠との重なりを考慮して広めに設定
        const scrollSpeed = 8; // スクロール速度（px/frame）

        let scrollX = 0;
        let scrollY = 0;

        // 右端判定（ウィンドウ右端±threshold以内）
        // 端を少し超えてもスクロール継続、ただし大きく超えた場合は発動しない
        if (e.clientX > windowWidth - threshold && e.clientX <= windowWidth + threshold) {
            scrollX = scrollSpeed;
            this.expandCanvasIfNeeded('right', canvas, pluginContent);
        }
        // 左端判定（ウィンドウ左端±threshold以内）
        else if (e.clientX >= -threshold && e.clientX < threshold) {
            scrollX = -scrollSpeed;
        }

        // 下端判定（ウィンドウ下端±threshold以内）
        // 端を少し超えてもスクロール継続、ただし大きく超えた場合は発動しない
        if (e.clientY > windowHeight - threshold && e.clientY <= windowHeight + threshold) {
            scrollY = scrollSpeed;
            this.expandCanvasIfNeeded('bottom', canvas, pluginContent);
        }
        // 上端判定（ウィンドウ上端±threshold以内）
        else if (e.clientY >= -threshold && e.clientY < threshold) {
            scrollY = -scrollSpeed;
        }
        ...
        if (scrollX !== 0 || scrollY !== 0) {
            ...
            this.edgeScrollState.isScrolling = true;
        } else {
    ...
    finishDrag() {
        ...
        if (this.virtualObjectDragState.hasMoved && this.vobjDragState.currentObject && this.vobjDragState.currentElement) {
            ...
            if (this.vobjDragState.dropClientX !== undefined && this.vobjDragState.dropClientY !== undefined) {
                // マウス移動量 + スクロール差分 = キャンバス上の実際の移動量
                deltaX = (this.vobjDragState.dropClientX - this.virtualObjectDragState.startX) + scrollDeltaX;
                deltaY = (this.vobjDragState.dropClientY - this.virtualObjectDragState.startY) + scrollDeltaY;
                ...
                // この場合、style.left/topには既にスクロール差分が含まれている
            ...
            if (this.virtualObjectDragState.dragMode === 'copy') {
                ...
                objectsToMove.forEach((targetObj, index) => {
                    ...
                    // 座標を整数に丸める（小数点を防止）
                    targetObj.vobjleft = Math.round(newLeft);
                    targetObj.vobjtop = Math.round(newTop);
                    targetObj.vobjright = Math.round(newLeft + width);
                    targetObj.vobjbottom = Math.round(newTop + height);
                    ...
                    // DOM要素の位置を更新（インクリメンタル更新：iframeを破棄しない）
                    const vobjIndex = this.virtualObjects.indexOf(targetObj);
                    this.updateVirtualObjectElementPosition(vobjIndex, {
                        left: targetObj.vobjleft,
                        top: targetObj.vobjtop,
                        right: targetObj.vobjright,
                        bottom: targetObj.vobjbottom
                    });
                });
            }
    ...
    applyBackgroundColor(color) {
        ...
        if (bgColor) {
            ...
            setTimeout(() => {
                ...
                // .virtual-list に背景色を適用（スクロール領域全体をカバー）
                const virtualList = document.querySelector('.virtual-list');
                if (virtualList) {
                    virtualList.style.backgroundColor = bgColor;
                }
    ...
    convertDecorationTagsToHTML(content) {
        ...
        while ((match = fontRegex.exec(html)) !== null) {
            ...
            if (fullMatch.toLowerCase() === '</font>') {
                ...
                if (value === '' && isSelfClosing) {
                    ...
                    // 開始タグ: spanを生成
                    result += this.createTadStyledSpan(attr, value);
        ...
        html = html.replace(/<invert>(.*?)<\/invert>/gi, `<span style="background-color: currentColor; color: ${DEFAULT_BGCOL}; padding: 0 2px;">$1</span>`);
     ...
     */
    extractTADXMLFromElement(element, xmlParts, fontState = { size: String(DEFAULT_FONT_SIZE), color: DEFAULT_CHCOL, face: '' }) {
        ...
        nodes.forEach(node => {
            if (node.nodeType === 3) { // Node.TEXT_NODE
                ...
                if (['underline', 'overline', 'strikethrough', 'bagchar', 'box', 'invert', 'mesh', 'noprint', 'ruby'].includes(nodeName)) {
                    ...
                    // 色はペアタグ方式: <font color="...">content</font>
                    // サイズ・フェイスは自己閉じタグ方式を維持
    ...
    async parseVirtualObjects() {
        try {
            ...
            // Promise.allSettledで個別エラーを許容
            const results = await Promise.allSettled(
                parsedObjects.map(obj => this.loadVirtualObjectMetadata(obj))
            );

            // 3. 元の順序を維持して配列に追加（失敗してもオブジェクトは追加）
            this.virtualObjects = [];
            parsedObjects.forEach((obj, index) => {
                if (results[index].status === 'rejected') {
                    logger.warn('[VirtualObjectList] メタデータ読み込み失敗:', obj.link_id, results[index].reason);
                }
                // メタデータ読み込み失敗時もオブジェクトは追加（メタデータなしで表示）
            ...
            if (window.parent && window.parent.tadjsDesktop && window.parent.tadjsDesktop.fileObjects) {
                ...
                if (jsonFile) {
                    ...
                    try {
                        ...
                        logger.debug('[VirtualObjectList] 親ウィンドウ経由で失敗:', parentError.message);

                        // クラウドフォールバック: cloudContextがある場合、親ウィンドウ経由でクラウドDL
                        if (this.cloudContext && this.cloudContext.tenantId) {
                            try {
                                const dlMessageId = this.generateMessageId('dl-cloud');
                                this.messageBus.send('download-cloud-to-local', {
                                    tenantId: this.cloudContext.tenantId,
                                    realId: baseFileId,
                                ...
                                if (dlResult.success) {
                                    ...
                                    logger.debug('[VirtualObjectList] メタデータ読み込み成功（クラウドDL経由）:', jsonFileName);
                                    return;
                                }
                            } catch (cloudError) {
                                logger.debug('[VirtualObjectList] クラウドフォールバック失敗、HTTP fetchにフォールバック:', cloudError.message);
                            }
                        }
                ...
                try {
                    ...
                    if (this.cloudContext && this.cloudContext.tenantId) {
                        try {
                            ...
                            logger.debug('[VirtualObjectList] クラウドフォールバック失敗:', cloudError.message);
                        }
                    }
    ...
    /**
     * 仮身要素の位置のみを更新（DOM再作成なし）
     * @param {number} vobjIndex - virtualObjects配列内でのインデックス
     * @param {Object} newPosition - 新しい位置 {left, top, right, bottom}
     */
    updateVirtualObjectElementPosition(vobjIndex, newPosition) {
        const canvas = document.querySelector('.virtual-canvas');
        if (!canvas) return;
        ...
        element.style.width = (newPosition.right - newPosition.left) + 'px';
        element.style.height = (newPosition.bottom - newPosition.top) + 'px';
    }
    ...
    removeVirtualObjectElement(vobjIndex) {
        ...
        const element = canvas.querySelector(`[data-vobj-index="${vobjIndex}"]`);
        if (element) {
            // 開いた仮身の場合、iframeを展開リストから削除
            const iframe = element.querySelector('iframe');
            if (iframe) {
                this.expandedIframes.delete(iframe);
            }
            element.remove();
        }
    }
     ...
     * 仮身要素を追加（DOM再作成なし）
     * @param {Object} obj - 仮身オブジェクト
     * @param {number} insertAt - 挿入位置（virtualObjects配列内でのインデックス）
     */
    ...
    /**
     * 全仮身要素のz-indexを再計算して更新
     */
    updateVirtualObjectElementZIndex() {
        const canvas = document.querySelector('.virtual-canvas');
        if (!canvas) return;
        ...
        allObjectsInOrder.forEach((obj, index) => {
            ...
            if (element) {
                if (obj.isBackground) {
                    ...
                    element.style.zIndex = 1000 + index - backgroundObjects.length;
                }
            }
        });
    }
    ...
    /**
     * 全仮身要素のdata-vobj-indexを再計算
     * 注意: link_idは重複可能（同じ実身への複数の仮身）なので、
     * DOM順でインデックスを割り当てる
     */
    updateVobjIndices() {
        const canvas = document.querySelector('.virtual-canvas');
        ...
        // DOM順で全仮身要素を取得し、インデックスを順番に割り当てる
        // （同じlink_idを持つ複数の要素がある場合も正しく処理される）
    ...
    renderVirtualObjects() {
        ...
        allObjectsInOrder.forEach((obj, index) => {
            // 開いた仮身と閉じた仮身を判定：VirtualObjectRendererに委譲
    ...
    attachVirtualObjectEventListeners(vobj, obj) {
        ...
        vobj.addEventListener('click', (e) => {
            ...
            // data-vobj-index属性からインデックスを取得（indexOf回避）
            const vobjIndex = parseInt(vobj.getAttribute('data-vobj-index'));
            // data-vobj-indexが取得できない場合は処理をスキップ（開いた仮身のiframe領域クリック時など）
            if (isNaN(vobjIndex)) {
                return;
            }
    ...
    createClosedVirtualObject(obj) {
        ...
        vobj.addEventListener('click', (e) => {
            ...
            // data-vobj-index属性からインデックスを取得（indexOf回避）
            const clickVobjIndex = parseInt(vobj.getAttribute('data-vobj-index'));
            this.selectVirtualObject(obj, vobj, e, clickVobjIndex);
        });
    ...
    /**
     * 仮身を選択状態にする
     * @param {Object} obj - 選択する仮身オブジェクト（参照用、実際はvobjIndexから取得）
     * @param {HTMLElement} element - 仮身のDOM要素
     * @param {Event} event - クリックイベント（Shiftキー判定用）
     * @param {number|null} vobjIndex - 仮身のインデックス（省略時はdata-vobj-indexから取得）
     */
    selectVirtualObject(obj, element, event, vobjIndex = null) {
        ...
        // vobjIndexが渡されていない場合はdata-vobj-index属性から取得
        if (vobjIndex === null) {
            vobjIndex = parseInt(element.getAttribute('data-vobj-index'));
        }

        // 配列から最新のオブジェクトを取得（クロージャの参照問題を回避）
        const currentObj = this.virtualObjects[vobjIndex];

        if (!currentObj) {
            logger.warn('[VirtualObjectList] 仮身が見つかりません: vobjIndex=', vobjIndex);
            return;
        }

        // 以降はobjではなくcurrentObjを使用
        obj = currentObj;
    ...
    makeVirtualObjectDraggable(vobjElement, obj) {
            ...
            if (this.vobjDragState.selectedObjects && this.vobjDragState.selectedObjects.length > 0) {
                ...
                virtualObjects = this.vobjDragState.selectedObjects.map(item => {
                    ...
                    return {
                        ...
                        heightPx: vobj.heightPx || DEFAULT_VOBJ_HEIGHT,
                ...
                virtualObjects = [{
                    ...
                    link_name: obj.link_name,
                    width: obj.width || 150,
                    heightPx: obj.heightPx || DEFAULT_VOBJ_HEIGHT,
        ...
        vobjElement.addEventListener('dragend', (e) => {
            ...
            if (e.dataTransfer.dropEffect === 'none') {
                ...
                // dropイベントでdropClientX/Yが設定されていない場合、dragendの座標を使用
                // （仮身の上でドロップした場合、dropイベントがvirtual-listに到達しない可能性がある）
                if (this.vobjDragState.dropClientX === undefined && e.clientX !== 0 && e.clientY !== 0) {
                    this.vobjDragState.dropClientX = e.clientX;
                    this.vobjDragState.dropClientY = e.clientY;
                    logger.debug('[VirtualObjectList] dragendの座標をdropClientX/Yに設定:', e.clientX, e.clientY);
                }
        ...
        vobjElement.addEventListener('mousedown', (e) => {
            ...
            // data-vobj-indexが取得できない場合は処理をスキップ（開いた仮身のiframe領域クリック時など）
            if (isNaN(this.vobjDragState.vobjIndex)) {
                return;
            }
            ...
            // data-vobj-index属性から取得済みのインデックスを使用（indexOf回避）
    ...
    makeVirtualObjectResizable(vobjElement, obj) {
        ...
        vobjElement.addEventListener('mousedown', (e) => {
            ...
            const onMouseUp = () => {
                ...
                // 開いた仮身のiframeは常に無効化されたままにするため、再有効化しない
                // リサイズ開始時に設定された pointer-events: none はそのまま維持
                ...
                // 横幅のみリサイズ（縦方向のリサイズなし）の場合は、元の高さを維持
                ...
                if (wasOpen !== isNowOpen) {
                    ...
                    // 既存の要素からvobjIndexを取得（再作成時にイベントハンドラが正しく動作するために必要）
                    let existingVobjIndex = parseInt(elementToReplace.getAttribute('data-vobj-index'));
                    // NaNの場合は配列から再計算（フォールバック）
                    if (isNaN(existingVobjIndex)) {
                    ...
                    this.recreateVirtualObjectTimer = setTimeout(() => {
                        ...
                        // 元のz-indexを新しい要素に設定
                        if (originalZIndex) {
                            newElement.style.zIndex = originalZIndex;
                        }
