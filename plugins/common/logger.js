...
(function(global) {
    ...
    // 親ウィンドウから設定を取得（プラグイン用）
    function initFromParent() {
        try {
            if (window.parent && window.parent !== window && window.parent.TADjsConfig?.log) {
                logManager.configure(window.parent.TADjsConfig.log);
            }
        } catch (e) {
            // クロスオリジンエラーの場合は無視
        }
